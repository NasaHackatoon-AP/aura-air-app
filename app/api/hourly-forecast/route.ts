...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`🌤️ Hourly Forecast API: Fetching for userId: ${userId}`);
    ...
    console.log(`📊 Hourly Forecast API: External status: ${response.status}`);

    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        `❌ Hourly Forecast API: External error: ${response.status} - ${errorText}`
      );
      ...
      console.log("🔄 Hourly Forecast API: Using fallback data");
      return NextResponse.json(fallbackData);
    }

    const data = await response.json();
    console.log("✅ Hourly Forecast API: Data received:", data);
