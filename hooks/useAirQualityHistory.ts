...
export function useAirQualityHistory({
  ...
  const fetchAirQualityHistory = useCallback(async () => {
    console.log(
      `🔍 useAirQualityHistory: Buscando dados para userId: ${userId} em ${location.city}, ${location.country} (${location.latitude}, ${location.longitude})`
    );
    setIsLoading(true);
    setError(null);
